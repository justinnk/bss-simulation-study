...
with open('results_graphs/stats.txt', 'w+') as file:
    file.write(
        '''
...
with open('results_graphs/validation_stats.txt', 'w+') as file:
    file.write(
        '''
...
plt.savefig('results_graphs/stations.png', dpi=300)

# write station numbers to file
with open('results_graphs/station_numbers.txt', 'w+') as file:
...
fig.savefig('results_graphs/roundtrips.png', dpi=300)
...
plt.xticks(rotation='90')
fig.subplots_adjust(bottom=0.3)
fig.savefig('results_graphs/arrivals_departures_absolute.png', dpi=300)

'''
# average arrivals and departures per station
fig, ax = plt.subplots()
fig.set_size_inches(12, 8)
ax.set_title('Average Arrivals and Departures from Stations per Day')
ax.set_xlabel('station name')
ax.set_ylabel('absolute frequency')
width = 0.35
dar = dataset.groupby('end_station_name').mean().reset_index()
dde = dataset.groupby('start_station_name').mean().reset_index()
ar = ax.bar(dar.index, dar.duration, width)
de = ax.bar(dar.index + width, dde.duration, width)
ax.set_xticks(dar.index + width / 2)
ax.set_xticklabels(stations.station_name.sort_values())
...
plt.xticks(rotation='90')
fig.subplots_adjust(bottom=0.3)
fig.savefig('results_graphs/arrivals_departures_average.png', dpi=300)
'''