# @author justinnk
# Produces Visualizations and data that can be used to validate the model
#
...
# validation/global.png
#   comparison of the number of trips per hour
...
sim_results = [file for file in os.listdir(os.path.join(results_dir)) if file.endswith('.csv') and file.startswith('Available')]

# number of hours the simulation is run for
simulation_hours = 24#int(settings['simulation_end_time'] / 60)
...
for index, s in stations.iterrows():
    ...
    availability_simulation = pd.read_csv(os.path.join(results_dir, getResults(index)), delimiter=',', usecols=[0,1,2,3], names=['time', 'value', 'stddev', 'stderr'])
    ...
    ax.set_xticks(range(0, 72), minor=True)
    # capacity
    ...
    ax.errorbar(range(0, 24), availability_validation, availability_variance_validation, label='#available (validation)', color='#06B500')
    # availability training set
    ax.errorbar(range(0, 24), availability_training, availability_variance_training, label='#available (trainig)', color='#3f6c91')
    # error
    ...
    fig.savefig(output_dir + '/validation/{0}_available.png'.format(s.station_name))
    plt.close(fig)
...
for file in os.listdir(results_dir):
    if file.startswith('Returns'):
            trips_simulation = pd.read_csv(os.path.join(results_dir, file), delimiter=',', usecols=[0,1,2,3], names=['time', 'value', 'stddev', 'stderr'])
...
ax.grid(which='major', linestyle='--')
ax.grid(which='minor', linestyle='--')
fig.savefig(output_dir + '/validation/global_returns.png')
...
ax.grid(which='major', linestyle='--')
ax.grid(which='minor', linestyle='--')
fig.savefig(output_dir + '/validation/global_retrievals.png')
