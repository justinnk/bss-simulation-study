 ...
 * all outputs to that folder. It is very similar to EvaluateBSS,
 * but evaluates 3 days instead of one.
 * 
...
public class EvaluateBSSMulti {
	
	public static void main(String[] args) throws SyntaxErrorExpection, IOException {
		EvaluateBSSMulti t = new EvaluateBSSMulti(Integer.parseInt(args[0]));
	}
	
	public EvaluateBSSMulti(int replications) throws SyntaxErrorExpection, IOException {
		...
		jSSTLScript script = new BSSFormulasMulti();
		...
		double phi_empty = checkFormula(script, "phi_empty", graph, graph.getNumberOfLocations(), getGlobalParams(), runs, trajPref);
		double phi_full = checkFormula(script, "phi_full", graph, graph.getNumberOfLocations(), getGlobalParams(), runs, trajPref);
	 ...
	 * @return
	 */
	private ArrayList<HashMap<String,Double>> getGlobalParams(){
	...
	private ArrayList<HashMap<String,Double>> getPhiProbParams(){
		...
		// day 1
		HashMap<String, Double> map = new HashMap<String, Double>();
		map.put("ts", Double.valueOf(0));
		map.put("te", Double.valueOf(1439));
		params.add(map);
		// day 2
		map = new HashMap<String, Double>();
		map.put("ts", Double.valueOf(1440));
		map.put("te", Double.valueOf(2879));
		params.add(map);
		// day 3
		map = new HashMap<String, Double>();
		map.put("ts", Double.valueOf(2880));
		map.put("te", Double.valueOf(4319));
		params.add(map);
		return params;
	}
	 ...
	 * the end. Otherwise a final time of 4319 for the formulas results in an error. The
	...
	public Signal readTrajectory(GraphModel graph, String filename) throws IOException {
		...
		times[times.length - 1] = 4320.0; // WORKAROUND
